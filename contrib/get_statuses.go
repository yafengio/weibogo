...
func GetStatuses(weibo *weibogo.Weibo, access_token string, userName string, userId int64, numStatuses int, timeout int) ([]*weibogo.Status, error) {
	...
	if userName == "" && userId == 0 {
		return nil, &weibogo.ErrorString{"userName和userId不可以都是无效值"}
	}

	// 计算需要启动的进程数
	if numStatuses <= 0 {
		return nil, &weibogo.ErrorString{"抓取微博数必须大于零"}
	}
	...
	output := make(chan *weibogo.Status, STATUSES_PER_PAGE*numThreads)
	...
	for i := 0; i < numThreads; i++ {
		...
		go func(page int) {
			var posts weibogo.Statuses
			var params weibogo.Params
			if userName != "" {
				params = weibogo.Params{"screen_name": userName, "count": STATUSES_PER_PAGE, "page": page}
			} else {
				params = weibogo.Params{"uid": userId, "count": STATUSES_PER_PAGE, "page": page}
			}
	...
	numTotalStatuses := 0
	statuses := make([]*weibogo.Status, 0, numThreads*STATUSES_PER_PAGE) // 长度为零但预留足够容量
	isTimeout := false
	t0 := time.Now()
	for {
		// 非阻塞监听output和done通道
	...
	if isTimeout {
		return nil, &weibogo.ErrorString{"抓取超时"}
	}
	...
	// 删除掉重复的微博
	sortedStatuses := make([]*weibogo.Status, 0, len(statuses))
...
type StatusSlice []*weibogo.Status
